...
def code():
    ...
    while True:
        ...
        if flag:
            true_text_surf, true_text_rect = text_objects(word1, menu_text)
        ...
        if button("Г О Т О В O", int(screen_width / 2 - 75),  int(screen_height / 1.2), 50, 50, click) :
            decod1 = ''
            decod2 = ''
            word1 = list(word)
            for i in range(len(word1)):
                ...
                if i < 2:
                    ...
                    decod2 += str(word1[i])
            print(decod1, decod2)
            word1 = coding(decod1) + coding(decod2)
            print(word1)
            print(text)
            if word1 == (text):
                screen.fill(WHITE)
